...
```
##### 6.5 如果calico有启动不成功的BUG问题
```shell script
[root@k8s-master01 ~]# kubectl describe pod calico-node-jrchv -n kube-system
最后显示:
...
重新刷新calico插件即可
[root@k8s-master01 ~]# kubectl apply -f calico.yaml
kube-system    calico-kube-controllers-5f6cfd688c-2vkwl   1/1     Running   9          35d
kube-system    calico-node-9nr4h                          1/1     Running   0          59s
kube-system    calico-node-g4k59                          1/1     Running   0          42s
kube-system    calico-node-qzqcd                          1/1     Running   0          22s
kube-system    calico-node-wt4j5                          1/1     Running   0          77s
```